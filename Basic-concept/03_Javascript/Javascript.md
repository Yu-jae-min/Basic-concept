...
- 콜백 패턴과 promise의 차이는?
  ...
  콜백 패턴은 말 그대로 비동기 처리를 위해 인자로 콜백 함수를 넘기는 패턴을 말한다. 콜백 패턴의 단점으로는 예외 처리도 어렵고 콜백 함수의 중첩으로 인해 가독성이 현저하게 떨어지는 콜백 지옥이 발생할 수 있다. 이런 단점을 보완하고자 es5에서 프로미스가 추가되었다. 프로미스는 new Promise를 통해 생성할 수 있으며 대기, 이행, 실패의 3가지 상태와 값을 가지고 있는 프로미스 객체를 반환한다. 또 인자로 콜백 함수를 받고 콜백 함수의 파라미터인 리졸브나 리젝티드를 통해 이행시나 실패 시의 반환 할 값을 지정할 수 있다. 또 반환하는 프로미스 객체의 상태 값을 통해 후속 처리를 할 수 있다. then을 통해 이행 시 후속 처리나 비동기 api를 동기적으로 요청할 수도 있고 catch를 통해 실패 시 예외 처리를 하거나 finally를 통해 성공실패 여부와 관계없이 후속 처리를 할 수 있다.
...
- 코드로 보는 실행 컨텍스트 이해 : 일반 함수

  ```js
  const Test = () => {
    fetch(url).then((response) => console.log("1"));
  ...
  1. 브라우저 스크립트를 로딩하고 실행하는 순간 전역 컨텍스트가 생성되어 콜스택(Call Stack)에 쌓이고 실행됩니다.
  2. 전역 컨텍스트 실행 단계 중 `console.log("3")` 호출문에 도달하면 콜스택에 함수 컨텍스트가 쌓이고 실행됩니다.
  3. `console.log("3")` 실행 완료 후 다시 전역 컨텍스트 실행을 이어서 진행하는데 이 때 `Test()` 호출문에 도달하면 다시 콜스택에 함수 컨텍스트가 쌓이고 실행됩니다.
  4. `Test()` 함수 컨텍스트의 실행 과정 중 `fetch()` 호출문에 도달하면 위와 마찬가지로 콜스택에 쌓입니다. 하지만 fetch 메소드는 비동기 통신으로 동작하므로 즉시 콜스택에서 제거되고 실제 코드 실행은 Web API, Task Queue, Event Loop에서 처리되게 됩니다.
  5. 비동기 처리가 이루어 지는 동안 `Test()` 함수 컨텍스트의 실행 단계는 이어서 진행됩니다. `console.log("2")` 호출문에 도달하면 콜스택에 함수 컨텍스트가 쌓이고 실행됩니다.
  6. 전역 컨텍스트까지 실행이 완료되어 콜스택이 비어있게 되면 콜스택을 감지하던 Event Loop가 비동기 처리가 완료 된 콜백을 콜스택에 밀어넣은 후 실행합니다. 이 때`console.log("1")`가 실행되게 됩니다.

  <br>

- 코드로 보는 실행 컨텍스트 이해 : 비동기 함수

  ```js
  const Test = async () => {
    await fetch(url).then((response) => console.log("1"));
  ...
  2. 전역 컨텍스트 실행 단계 중 `console.log("3")` 호출문에 도달하면 콜스택에 함수 컨텍스트가 쌓이고 실행됩니다.
  3. `console.log("3")` 실행 완료 후 다시 전역 컨텍스트 실행을 이어서 진행하는데 이 때 `Test()` 호출문에 도달하면 다시 콜스택에 함수 컨텍스트가 쌓이고 실행됩니다.
  4. `Test()` 함수 컨텍스트의 실행 과정 중 `fetch()` 호출문에 도달하면 위와 마찬가지로 콜스택에 쌓입니다. 이 때 `await` 키워드를 만나게 되면 자신을 포함한 `async` 함수의 실행을 중지하고 await 결과값을 기다리게 됩니다. 이 떄 결과값으로 `console.log("1")` 이 실행됩니다.

     > 💡 async 함수 내부에 await 키워드를 만나게 되면 일반 비동기 처리와 마찬가지로 해당 async 함수 자체(위 예시에서 Test)를 Web API로 넘기게 됩니다. 그렇기 떄문에 async 키워드가 붙지 않은 코드들은 이어서 실행되게 됩니다. Web API로 넘어간 async 함수는 await 결과 값을 연산하고 연산이 종료되면 콜스택을 감시하고 있던 Event Loop가 연산이 종료된 async 함수 컨텍스트를 비어있는 콜스택에 밀어넣고 await 키워드가 있던 위치부터 재실행됩니다.

  5. 그 후 다시 `Test()` 함수 컨텍스트의 실행 과정을 이어서 진행하며 `console.log("2")`를 호출합니다.
