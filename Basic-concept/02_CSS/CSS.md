...
- flex-shrink
  ...
  flex-shrink는 flex item이 flex-basis의 값보다 작아질 수 있는지를 결정하는 속성이다. 디폴트 값은 1로 작아질 수 있는 것을 뜻한다. 0으로 설정할 경우 flex-basis보다 작아지지 않기 때문에 고정 폭의 컬럼을 만들 수 있다.
...
### # **이미지 리플레이스먼트(Image Replacement)에 대해 설명해주세요**
...
- 이미지 리플레이스먼트(Image Replacement) 기법 적용 방법
