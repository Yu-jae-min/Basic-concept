...
- 어떤 사이트가 잘 만든 사이트 같은가 (구체적 답변을 원함) : 웹 사이트 제작 목적에 맞게 만들어진 사이트가 잘 만들어진 사이트라고 생각한다. 예를 들어 이커머스 사이트의 경우 검색 엔진 최적화가 너무나 중요한데

- 그렇다면 어떤 사이트가 못 만든 사이트 같은가 : 반대로 웹 사이트 제작 목적에 부합하지 못하는 사이트가 잘 못 만든 사이트라고 생각한다.
