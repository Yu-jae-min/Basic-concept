...
- 웹 크롤링이란 웹 페이지의 원하는 데이터를 수집해서 분류하고 저장하는 것을 뜻한다.
...
- 오버라이딩(Overriding) : 상위 클래스가 가지고 있는 메소드를 하위 클래스가 재정의하여 사용하는 방식이다.
  자바스크립트의 클래스 사용 중 extends를 통해 상속을 구현할 때 상속받는 클래스에 super 함수를 사용하여 오버라이딩 할 수 있다.
  클래스 바디에서 호출하여 메소드 오버라이딩을 할 수 있으며 constructor내부에서 호출하여 생성자 오버라이딩도 가능하다.

- 오버로딩(Overloading) : 같은 이름의 함수를 여러 개 만든 뒤 각각의 함수에 매개변수의 타입, 갯수를 다르게 지정하여
  호출 시 전달되는 인자에 타입이나 갯수에 따라 실행 될 함수를 매칭하여 다른 출력 값을 받는 방식을 말한다.
