...
- 순수 함수란?
  ...
  동일한 인자가 전달되면 항상 동일한 값을 반환하고, 외부에 영향을 주거나 받지도 않으며 사이드 이펙트가 발생하지 않는 함수를 얘기한다.

<br>

- 순수 함수를 사용하는 이유

  반환되는 결과가 예측 가능하므로 실행 시점이 중요하지 않고, 다른 순수 함수들과 조합하여 사용하는 것이 용이하여 재사용성이 증가한다는 장점이 있다.
