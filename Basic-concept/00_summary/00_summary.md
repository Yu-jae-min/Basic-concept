...
- 페이지별 기능 구현 방법
  ...
  - 공통
    ...
    - 페이지 접근 권한 및 헤더 : getStaticProps를 활용하여 로그인 유저와 비로그인 유저 접근 권한 분기 처리 및 헤더 노출 처리, app 디렉토리에서 component를 감싸는 HOC(고차 컴포넌트)를 만든 뒤 getStaticProps를 통해 app으로 전달 된 props를 고차 컴포넌트로 전달하여 처리

    - 이미지 리사이징 : 캔버스를 활용하여 이미지를 리사이징했다. 캔버스를 통해 리사이징한 이미지 blob을 업로드하여 이미지를 저장하였다.
