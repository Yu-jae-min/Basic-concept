...
- eslint란?
  ...
  자바스크립트 언어와 소스코드를 분석하는 도구로 코드에 특정 스타일과 규칙을 적용해 에러를 찾고 패턴을 적용시킬 수 있는 분석 툴이다. 코드 구현 방식을 지정하여 에러를 사전에 방지하고 코드 퀄리티를 보장하도록 도와준다. 또한 prettier와 같이 코드 스타일과 관련 된 코드 포멧팅도 지원한다.
...
- 타입 호환성이란 구조적 서브 타이핑을 기반으로 두 타입에 멤버만으로 타입이 호환되는지 판별하여 관계시키는 규칙이다. 타입 호환성을 통해 예상치 못한 에러를 방지하여 안정성을 향상시켰다. 타입 별 규칙의 예시로는 인터페이스의 경우 멤버의 속성과 값 등을 비교하고 함수의 경우 파라미터와 반환 타입 등을 비교하고 제네릭의 경우 전달받는 타입을 비교한다.

  (참고 : https://velog.io/@boyeon_jeong/%ED%83%80%EC%9E%85%EC%8A%A4%ED%81%AC%EB%A6%BD%ED%8A%B8%EC%9D%98-%ED%83%80%EC%9E%85-%ED%98%B8%ED%99%98-%ED%83%80%EC%9E%85%EC%9D%98-%EA%B5%AC%EC%A1%B0%EC%A0%81-%ED%98%B8%ED%99%98%EC%84%B1%ED%95%A8%EC%88%98-%EC%A0%9C%EB%84%A4%EB%A6%AD-%EC%9D%B4%EB%84%98)
...
- 타입 별칭은 특정 타입에 별칭을 부여하여 새로운 타입을 만들어내는 타입이다. 인터페이스와 같이 객체 타입을 지정할 수 있고 이 외에도 객체 타입이 아닌 원시 타입, 유니온 타입, 튜플 타입 등을 지정할 수 있다.
...
(2) 생성 타입 형태 : 타입 별칭은 객체 형태의 타입 외에도 원시 값(원시 타입 x 값 o), 유니온 타입, 튜플 타입 등을 지정할 수 있고 인터페이스는 객체 형태의 타입을 지정한다.
...
- HTTP 버전별 차이 (참고: https://withbundo.blogspot.com/2021/02/http-http-10-http-11.html)
  ...
  (3) HTTP/1.1 : 동시에 여러 개(최대 5개)의 요청과 각각의 응답을 받을 수 있는 파이프라이닝 추가, 버츄얼 호스팅(하나의 IP에 여러 도메인)을 가능하게 하는 호스트 헤더 추가, 한 커넥션 당 여러개의 요청을 할 수 있는 커넥션 유지 기능 추가, 현재 HTTP/1.1가 가장 많이 사용
